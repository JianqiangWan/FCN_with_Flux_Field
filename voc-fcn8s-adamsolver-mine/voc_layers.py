...
class VOCSegDataLayer(caffe.Layer):
    ...
    def setup(self, bottom, top):
        ...
        if len(top) != 4:
            raise Exception("Need to define two tops: data and label, flux, flux_weight.")
    ...
    def reshape(self, bottom, top):
        ...
        self.label, self.flux, self.flux_weight = self.load_label(self.indices[self.idx])
        # reshape tops to fit (leading 1 is for batch dimension)
        top[0].reshape(1, *self.data.shape)
        top[1].reshape(1, *self.label.shape)
        top[2].reshape(1, *self.flux.shape)
        top[3].reshape(1, *self.flux_weight.shape)
    ...
    def forward(self, bottom, top):
        ...
        top[2].data[...] = self.flux
        top[3].data[...] = self.flux_weight
    ...
    def load_label(self, idx):
        ...
        flux, flux_weight = label2flux(label)

        label = label[np.newaxis, ...]
        # flux = flux[np.newaxis, ...]
        flux_weight = flux_weight[np.newaxis, ...]

        return label, flux, flux_weight
